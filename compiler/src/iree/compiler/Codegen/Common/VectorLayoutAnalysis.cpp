...
ChangeResult DistributionLayout::resolveWithPossibleConflict(
  ...
  if (!opOperand.get().hasOneUse() && !vectorLayout &&
      llvm::isa_and_nonnull<arith::ConstantOp, vector::StepOp, vector::CreateMaskOp>(
...
static void propagateLayoutToContractionOp(
  ...
  if (!lhs->isUninitialized() && !rhs->isUninitialized()) {
    ...
    VectorLayoutInterface inferredResLayout = lhsLayout.getRecombinedLayout({lhsLayout, rhsLayout}, {lhsMap, rhsMap}, resMap);
  ...
  mask.getBody()->walk([&](Operation* op) {
    if(vector::ContractionOp contract = dyn_cast<vector::ContractionOp>(op)){
      ...
      if(!lhs->isUninitialized() && !rhs->isUninitialized()){
        ...
        llvm::errs() << "contract=" << contract << "\n";
        llvm::errs() << "lhsLayout=" << layouts[0] << "\n";
        llvm::errs() << "rhsLayout=" << layouts[1] << "\n";
        llvm::errs() << "inferredResLayout=" << inferredMaskLayout << "\n";
        DistributionLayout *maskLayout = getLatticeElement(mask.getMask());
        ChangeResult changed = maskLayout->resolveWithPossibleConflict(inferredMaskLayout, mask->getOpOperand(0));
        update(maskLayout, changed);
      }
    }
    if(vector::MultiDimReductionOp reduce = dyn_cast<vector::MultiDimReductionOp>(op)){
      const DistributionLayout *src = getLatticeElement(reduce.getSource());
      if(!src->isUninitialized()){
        DistributionLayout *maskLayout = getLatticeElement(mask.getMask());
        ChangeResult changed = maskLayout->resolveWithPossibleConflict(src->getLayout(), mask->getOpOperand(0));
        update(maskLayout, changed);
      }
    }
  });
